...
spec_segment='ACLOUD_P5_RF05_hl09'


cat = ac3airborne.get_intake_catalog()
list(cat.Polar5.CLOUD_TOP_HEIGHT)
ds_cloud_top_height = cat['Polar5']['CLOUD_TOP_HEIGHT'][campaign].to_dask()
ds_cloud_top_height
...
bins=stack.groupby_bins(group=stack*1e-3,bins=np.arange(0,(ds_cloud_top_height_sel.cloud_top_height.max()*1e-3)+0.005,0.005)).count()
ds_amali_a_sel_fraction=bins/len(ds_cloud_top_height_sel.time)
plt.figure()
plt.plot(ds_amali_a_sel_fraction)

fig, (ax1) = plt.subplots(1)
ax1.plot(ds_amali_a_sel_fraction,np.arange(0.0025,(ds_cloud_top_height_sel.cloud_top_height.max()*1e-3)+0.005,0.005))
